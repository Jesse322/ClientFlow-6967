...
import { getPlanYear } from "@/lib/planYear";
import { parseISO, differenceInDays, isValid } from "date-fns";
import { toast } from "sonner";
import type { AirtableRecord, Deliverable } from "@/lib/types";
import { Plus, Search, X, ChevronDown, ChevronUp, Calendar, Pencil, Trash2, CheckSquare, Square, XCircle, CheckCircle2, MessageSquare, User, Mail, UserCheck } from "lucide-react";
import { ExpandCard } from "@/components/ui/expand-card";
import { NotesLog } from "@/components/notes-log";
import { useSession } from "@/lib/session";

// ── Completion date display — color-coded by on-time vs late ─────────────────
...
const PHASES = ["All", "Pre-Renewal", "Marketing", "Implementation", "Post-Renewal", "Compliance"];
...
export default function DeliverablesPage() {
  ...
  const { data: items, loading, reload } = useDeliverables();
  ...
  const clientRenewalMap = useMemo(() => {
    const map: Record<string, string> = {};
    (clients || []).forEach((c) => { if (c.fields["Renewal Date"]) map[c.id] = c.fields["Renewal Date"]!; });
    return map;
  }, [clients]);
  ...
  const availablePlanYears = useMemo(() => {
    ...
    (items || []).forEach((d) => {
      ...
      const y = getPlanYear(d.fields["Deadline"], cid ? clientRenewalMap[cid] : undefined, d.fields["Renewal Timeline Phase"], d.fields["Deliverable Name"]);
      if (y !== null) years.add(y);
    });
    return Array.from(years).sort((a, b) => b - a);
  }, [items, clientRenewalMap]);

  const filtered = useMemo(() => {
    let list = (items || []).filter((i) => i.fields["Renewal Timeline Phase"] !== "Compliance" && officeClientIds.has(i.fields["Client"]?.[0] ?? ""));
    ...
    if (phaseFilter !== "All") list = list.filter((i) => i.fields["Renewal Timeline Phase"] === phaseFilter);
    if (clientFilter !== "All") list = list.filter((i) => i.fields["Client"]?.[0] === clientFilter);
    if (planYearFilter !== "All") {
      list = list.filter((i) => {
        const cid = i.fields["Client"]?.[0];
        const y = getPlanYear(i.fields["Deadline"], cid ? clientRenewalMap[cid] : undefined, i.fields["Renewal Timeline Phase"], i.fields["Deliverable Name"]);
        return y !== null && String(y) === planYearFilter;
      });
    }
    list = [...list].sort((a, b) => {
      let cmp = 0;
    ...
    return list;
  }, [items, search, statusFilter, typeFilter, phaseFilter, clientFilter, planYearFilter, showCompleted, sortBy, sortDir, clientMap, clientRenewalMap, memberMap, officeClientIds]);
  ...
  const doBulkDelete = async () => {
    ...
    try {
      ...
      toast.success(`Deleted ${selected.size} deliverable${selected.size !== 1 ? "s" : ""}`);
      setSelected(new Set());
      reload();
  ...
  const nonComplianceItems = useMemo(
    () => (items || []).filter((i) => i.fields["Renewal Timeline Phase"] !== "Compliance"),
    [items]
  );

  const counts = useMemo(() => ({
    overdue: nonComplianceItems.filter((i) => (daysUntil(i.fields["Deadline"]) ?? 0) < 0 && i.fields["Status"] !== "Completed").length,
    inProgress: nonComplianceItems.filter((i) => i.fields["Status"] === "In Progress").length,
    upcoming: nonComplianceItems.filter((i) => { const d = daysUntil(i.fields["Deadline"]); return d !== null && d >= 0 && d <= 30 && i.fields["Status"] !== "Completed"; }).length,
  }), [nonComplianceItems]);
    ...
    <div>
      ...
      <div className="flex gap-2 mb-4 flex-wrap">
        {STATUSES.slice(1).map((s) => {
          const count = nonComplianceItems.filter((i) => i.fields["Status"] === s).length;
          return (
      ...
      <div className="mb-4 space-y-2">
        ...
        <div className="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap">
          ...
          <Select value={phaseFilter} onValueChange={setPhaseFilter}>
            <SelectTrigger className="h-9 text-sm w-full sm:w-36"><SelectValue placeholder="Phase" /></SelectTrigger>
          ...
          {availablePlanYears.length > 0 && (
            <Select value={planYearFilter} onValueChange={setPlanYearFilter}>
              <SelectTrigger className="h-9 text-sm w-full sm:w-28">
                <Calendar className="w-3.5 h-3.5 mr-1 text-slate-400" />
                <SelectValue placeholder="PY" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="All">All years</SelectItem>
                {availablePlanYears.map((y) => <SelectItem key={y} value={String(y)}>PY {y}</SelectItem>)}
              </SelectContent>
            </Select>
          )}
        ...
        <div className="flex items-center gap-2 mb-3 px-4 py-2.5 bg-sky-50 border border-sky-200 rounded-xl flex-wrap">
          ...
          <div className="flex items-center gap-1">
            ...
            <Select onValueChange={async (memberId) => {
              try {
                await Promise.all([...selected].map((id) => updateDeliverable(id, { "Assigned Team Members": memberId ? [memberId] : [] })));
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-slate-400 text-sm">No deliverables match your filters</div>
      ) : (
        <>
          {/* Mobile cards */}
          <div className="sm:hidden space-y-2">
            {filtered.map((item) => {
              ...
              const renewal = clientId ? clientRenewalMap[clientId] : undefined;
              const planYear = getPlanYear(item.fields["Deadline"], renewal, item.fields["Renewal Timeline Phase"], item.fields["Deliverable Name"]);
              ...
              return (
                <SwipeableRow key={item.id} onEdit={() => setEditItem(item)} onDelete={() => handleDelete(item.id)}>
                  <ExpandCard
                    color={isOverdue ? "#ef4444" : item.fields["Status"] === "Completed" ? "#10b981" : "#0ea5e9"}
                    className={cn(isSelected ? "border-sky-300 bg-sky-50/50" : isOverdue ? "border-red-200 bg-red-50/30" : "")}
                    expandedContent={
                      <div className="space-y-2">
                        <div className="flex gap-3 flex-wrap text-xs text-muted-foreground">
                          {item.fields["Deadline"] && <span>Deadline: {formatDate(item.fields["Deadline"])}</span>}
                          {item.fields["Renewal Timeline Phase"] && <span>Phase: {item.fields["Renewal Timeline Phase"]}</span>}
                          {planYear && <span>Plan Year: {planYear}</span>}
                        </div>
                        {assignedId && (
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-2">
                          <p className="text-sm font-semibold text-foreground leading-snug flex-1 min-w-0">{item.fields["Deliverable Name"]}</p>
                        ...
                        <div className="flex items-center gap-2 flex-wrap mt-2">
                          ...
                          {planYear && <span className="text-xs text-muted-foreground font-medium">PY {planYear}</span>}
          ...
          {/* Desktop table — cols: checkbox | name | client | status | type | assigned | deadline | actions */}
                    ...
                    <SwipeableRow onEdit={() => setEditItem(item)} onDelete={() => handleDelete(item.id)}>
                      <div className="grid grid-cols-[auto_2fr_1.2fr_1fr_0.8fr_1fr_1fr_auto] gap-2 px-5 py-3.5 hover:bg-slate-50/50 items-center">
                        ...
                        <div className="min-w-0">
                          ...
                          {item.fields["Renewal Timeline Phase"] && (
                            <p className="text-xs text-slate-400 truncate mt-0.5">{item.fields["Renewal Timeline Phase"]}</p>
                          )}
                        </div>
      ...
      <ConfirmDialog
        ...
        title={`Delete ${selected.size} deliverable${selected.size !== 1 ? "s" : ""}?`}
        description="This will permanently delete the selected deliverables."
